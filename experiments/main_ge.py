...
def get_prior_model(checkppoint):
    model = ConditionalPointCloudDiffusionModel_ge(
        beta_start=0.1,
        beta_end=0.2,
        beta_schedule="linear",  # 或者 "custom"
        point_cloud_model="pvcnn",  # 根据实际情况选择模型
        point_cloud_model_embed_dim=64,
    )

    # 加载 checkpoint
    checkpoint = torch.load(checkppoint)
    model.load_state_dict(checkpoint['model_state_dict'])

    # 如果有其他内容，比如优化器、epoch等
