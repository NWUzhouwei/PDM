...
def get_character_embeddings(embed_dim, categories, device):
    """
    字符嵌入函数。输入为类别字符串，输出对应的嵌入表示，维度与 embed_dim 一致。
    """
    # 创建字符到索引的映射
    unique_characters = set(''.join(categories))
    char_to_idx = {char: idx for idx, char in enumerate(unique_characters)}
    num_chars = len(char_to_idx)

    # 使用嵌入层来生成字符嵌入
    char_embedding = nn.Embedding(num_chars, embed_dim//2).to(device)

    # 生成类别的嵌入
    embeddings = []
    for category in categories:
        char_indices = torch.tensor([char_to_idx[char] for char in category], device=device)
        char_emb = char_embedding(char_indices)  # 获取字符嵌入
        # 平均字符嵌入作为类别嵌入
        category_emb = char_emb.mean(dim=0)  # 平均
    ...
    emb = np.log(10000) / (half_dim - 1)
    emb = torch.from_numpy(np.exp(np.arange(0, half_dim) * -emb)).float().to(device)
    embeddings = (embeddings[:, None] * emb[None, :])
    embeddings=torch.squeeze(embeddings, dim=1)# 广播相乘
    emb_output = torch.cat([torch.sin(embeddings), torch.cos(embeddings)], dim=1)

    # 如果需要，填充为偶数维度
    if embed_dim % 2 == 1:
        emb_output = nn.functional.pad(emb_output, (0, 1), "constant", 0)

    assert emb_output.shape == torch.Size([len(categories), embed_dim])
    emb_output=emb_output.mean(dim=0, keepdim=True)
    return emb_output
...
class Mamba3D_GE(nn.Module):
    ...
    def forward(self, inputs, t,condition):
        ...
        condition_emb=get_character_embeddings(self.encoder_dims,condition,inputs.device).float()
        # condition_emb=self.mlp(condition)
        condition_emb=condition_emb[:,:,None].expand(12, -1, self.num_group).permute(0, 2, 1).contiguous()
        pts = inputs[:, :3, :].permute(0, 2, 1).contiguous()  # (B, 3, N)
        # features = inputs[:, 3:, :].permute(0, 2, 1).contiguous()
        condition=t_emb+condition_emb
        # (B, 3 + S, N)
        B = pts.size(0)
        N = pts.size(1)
        # neighborhood
        ...
        # transformer
        # 32 129 384
        feature_list = self.blocks(center, x, condition)  # enter transformer blocks
