...
class Mamba3D(nn.Module):
    ...
    def forward(self, inputs,t):
        ...
        features = inputs[:,3:,:].permute(0,2,1).contiguous()  # (B, 3 + S, N)
        B=pts.size(0)
        N = pts.size(1)
        #neighborhood
        neighborhood, center,group_feature = self.group_divider(pts,features) # B G K 3
        ...
        x_global_feature = torch.cat((x_max_feature, x_avg_feature, features.permute(0,2,1).contiguous()), 1)
