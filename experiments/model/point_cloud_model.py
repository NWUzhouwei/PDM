...
from .mamba3d.models.Mamba3D_ge import Mamba3D_GE
...
class PointCloudModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        elif self.model_type == 'mamba3d_ge':
            self.autocast_context = torch.autocast('cuda', dtype=torch.float32)
            self.model = Mamba3D_GE(
                num_classes=out_channels
            )
    ...
    # def forward(self, inputs: Tensor, t: Tensor,condition: Tensor=None) -> Tensor:
    #     """Receives input of shape (B, N, in_channels) and returns output
    #     of shape (B, N, out_channels)"""
    #     with self.autocast_context:
    #         return self.model(inputs.transpose(1, 2), t,condition).transpose(1, 2)
...
class PointCloudModel_ge(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
        self,
        model_type: str = "pvcnn",
