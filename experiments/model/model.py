...
from .point_cloud_model import PointCloudModel, PC2_PVDFusionModel,PointCloudModel_ge
...
class ConditionalPointCloudDiffusionModel(PointCloudProjectionModel):
    ...
    def forward_train(
        ...
        # noise_pred = self.point_cloud_model(x_t, timestep,condition)
    ...
    def forward_sample(
        ...
        # condition = self.get_text_condition("a photo of airplane")
        # condition="car"
        for i, t in enumerate(progress_bar):
            ...
            # noise_pred = self.point_cloud_model(
            #     x_t, t.reshape(1).expand(B),condition)
...
class ConditionalPointCloudDiffusionModel_ge(PointCloudProjectionModel):
    def __init__(
        self,
        beta_start: float,
        beta_end: float,
        ...
        **kwargs,  # projection arguments
        ...
        # Create point cloud model for processing point cloud at each diffusion step
        self.point_cloud_model = PointCloudModel_ge(
            model_type=point_cloud_model,
            embed_dim=point_cloud_model_embed_dim,
            in_channels=self.in_channels,
            out_channels=self.out_channels,
