...
class PointCloudProjectionModel(ModelMixin):
    ...
    def __init__(
        ...
        self.consistent_center = consistent_center

        # self.textmodel, self.token, self.preprocess = open_clip.create_model_and_transforms("ViT-B-32", pretrained=None)
        # self.textmodel = self.textmodel.to(self.device)
    ...
    def get_text_condition(self,text):
        ...
        text_features = self.textmodel.encode_text(text_tokens).to(self.device)
    ...
    def get_input_with_conditioning(
        ...
        if self.use_local_conditioning:
            ...
            # HACK here to support batch of cameras
            if isinstance(camera, list):
                local_features_proj = []
                for i in range(len(camera)):
                    assert isinstance(camera[i], CamerasBase)
                    local_feature_proj = self.surface_projection(points=x_t[i:i+1, :, :3],
                        camera=camera[i], local_features=local_features[i:i+1, ...])
                    local_features_proj.append(local_feature_proj)
                local_features_proj = torch.cat(local_features_proj, dim=0)
            else:
