...
def visualize_distance_transform(
    ...
    images = images[
        :, -1:
    ]  # (B, 1, H, W)  # get only distances (not vectors for now, for simplicity)
    image_grid = make_grid(
        images, nrow=int(math.sqrt(len(images))), pad_value=1, normalize=True
    )
...
def visualize_image(
    ...
    if images.shape[3] in [1, 3, 4]:  # convert to (B, C, H, W)
        images = images.permute(0, 3, 1, 2)
    if images.shape[1] in [
        3,
        4,
    ]:  # normalize (single-channel images are not normalized)
        images[:, :3] = (
            images[:, :3] * std + mean
        )  # denormalize (color channels only, not alpha channel)
    if images.shape[1] == 4:  # normalize (single-channel images are not normalized)
        image_alpha = images[:, 3:]  # (B, 1, H, W)
        bg_color = (
            torch.tensor([230, 220, 250], device=images.device).reshape(1, 3, 1, 1)
            / 255
        )
        images = images[:, :3] * image_alpha + bg_color * (
            1 - image_alpha
        )  # (B, 3, H, W)
...
def visualize_pointcloud_batch_pytorch3d(
    ...
    if cameras is None:
        ...
        colors = (
            ...
            else pointclouds.features_padded().tile(F, 1, 1)
        )  # (F * B, num_points, 3)

        # Initialize batch of cameras
        cameras = OrthographicCameras(
            focal_length=(0.25 * scale_factor), device=device, R=R, T=T
        )
