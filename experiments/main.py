...
def main(cfg: ProjectConfig):
    ...
    print(f"Parameters (train): {sum(p.numel() for p in model.parameters() ):_d}")
        ...
        for i, batch in enumerate(progress_bar):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process and (
                    train_state.step % cfg.run.checkpoint_freq == 0
                ):
                    checkpoint_dict = {
                ...
                # Visualize
                if (cfg.run.vis_freq > 0) and (
                    train_state.step % cfg.run.vis_freq
                ) == 0:
                    visualize(
                        cfg=cfg,
                        model=model,
...
def visualize(
    *,
    ...
    model.eval()
    metric_logger = training_utils.MetricLogger(delimiter="  ")
    progress_bar: Iterable[FrameData] = metric_logger.log_every(
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        torch.save(
            {"output": output, "all_outputs": all_outputs, "batch": batch}, filename
        )
        ...
        for i in range(len(output)):
            ...
            if not isinstance(
                gt_pointcloud, Pointclouds
            ):  # HACK: support for Tensor pcd
                gt_pointcloud = Pointclouds(gt_pointcloud[None])
            ...
            filename = filestr.format(
                dir="pointclouds", name="pointclouds", i=i, ext="html"
            )
            fig.write_html(filename)

            # Add to W&B
            filename_wandb = filestr_wandb.format(
                dir="pointclouds", name="pointclouds", i=i
            )
            ...
            for name, pointcloud in (("gt", gt_pointcloud), ("pred", pred_pointcloud)):
                ...
                # # Render gt/pred point cloud from rotating view NOTE: removed
            ...
            # # Render point cloud diffusion evolution NOTE: removed
    ...
    *,
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        for sample_idx in range(cfg.run.num_samples):
            ...
            base_save_path = Path("./saved_pointclouds")
            ...
            for i in range(len(output)):
                ...
                (output_dir / "metadata" / sequence_category).mkdir(
                    exist_ok=True, parents=True
                )
                (output_dir / "evolutions" / sequence_category).mkdir(
                    exist_ok=True, parents=True
                )

                pred_all_pointclouds = all_outputs[i]

                batch_save_path = output_dir / "sequence"/sequence_category
                ...
                steps_first = [0, 22, 45, 67, 89]

                    # 后10步（91-100），采样5步
                steps_last = [91, 93, 95, 97, 100]  #
                step_indices = steps_first + steps_last
                for j in step_indices:
                    # 获取第 i 步的点云
                    current_pointcloud = pred_all_pointclouds[j]
                    save_filename = sequence_name+f"pointcloud_step_{j + 1}"
                    # save_path = batch_save_path/save_filename
                    # 创建一个 Pointclouds 对象
                    # # pointcloud = Pointclouds(points=current_pointcloud)
                    io.save_pointcloud(
                        data=current_pointcloud,
                        path=filestr.format(
                            dir="sequence",
                    ...
                    # save_ply(current_pointcloud, save_path)

                    # print(f"保存了第{j + 1}个步骤的点云到 {save_path}")
